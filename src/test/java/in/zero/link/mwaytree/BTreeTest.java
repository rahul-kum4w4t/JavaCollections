...
public class BTreeTest {
    ...
    final static BTree<Integer> numTreeAsc = new BTree<>(Integer.class, 5);
    final static BTree<Integer> numTreeDesc = new BTree<>(Integer.class, 4, true);
    ...
    @Test
    void testAPIs() {
        assertEquals("[21, 7, 1, 5, 15, 10, 11, 17, 20, 35, 25, 28, 33, 71, 42, 51, 73, 80, 90]", Arrays.toString(numTreeAsc.preOrder()), "");
        assertEquals("[1, 5, 10, 11, 7, 17, 20, 15, 25, 28, 33, 42, 51, 35, 73, 80, 90, 71, 21]", Arrays.toString(numTreeAsc.postOrder()), "");
        assertEquals("[1, 5, 7, 10, 11, 15, 17, 20, 21, 25, 28, 33, 35, 42, 51, 71, 73, 80, 90]", Arrays.toString(numTreeAsc.inOrder()), "");
        ...
        assertEquals("[90, 80, 73, 71, 51, 42, 35, 33, 28, 25, 21, 20, 17, 15, 11, 10, 7, 5, 1]", Arrays.toString(numTreeDesc.inOrder()), "");
    ...
    void removalTest() {
        ...
        data.forEach(val -> {
            assertEquals(val, numTree5.remove(val), "After removal, value not returned");
            assertFalse(numTree5.search(val), "After removal value still present in the tree");
            testTreeIntegrityAsc(numTree5, val);
        });

        final BTree<Integer> numTree6RightBias = new BTree<>(Integer.class, 6);
        data.forEach(numTree6RightBias::add);

        data.forEach(val -> {
            assertEquals(val, numTree6RightBias.remove(val), "After removal, value not returned");
            assertFalse(numTree6RightBias.search(val), "After removal value still present in the tree");
            testTreeIntegrityAsc(numTree6RightBias, val);
        });

        final BTree<Integer> numTree6LeftBiasDesc = new BTree<>(Integer.class, 6, true, true);
        data.forEach(numTree6LeftBiasDesc::add);

        data.forEach(val -> {
            assertEquals(val, numTree6LeftBiasDesc.remove(val), "After removal, value not returned");
            assertFalse(numTree6LeftBiasDesc.search(val), "After removal value still present in the tree");
            testTreeIntegrityDesc(numTree6LeftBiasDesc, val);
        });
        ...
        for (MwayNode<Integer> node : nodes) {
            if (node != null) {
                ...
                if (node.children[0] != null) {
                    assertFalse(Arrays.stream(Arrays.copyOfRange(node.children, 0, node.counter + 1)).anyMatch(Objects::isNull));
                    Arrays.stream(node.children).filter(Objects::nonNull).forEach(stack::add);
                } else {
                    leafCount++;
                }
            }
        }
        if (leafCount > 0 && leafCount != nodes.size()) {
            throw new RuntimeException("All the leaf nodes are not at same level");
        }
        if (!stack.isEmpty()) testLevel(stack);
