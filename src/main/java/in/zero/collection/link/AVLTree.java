 ...
 * restricted by the JVM heap size) ------------------------------------------
 * 1. AVL tree is strictly balanced tree 2. Can have N number of rotations to
 * balance any node
 *
...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T> {
	...
	public AVLTree(boolean inverse) {
		super(inverse);
	}
	...
	/**
	 * Get the direction of child node which decides the height of the concerned
	 * node
	 *
	...
	private int getDominatingChildDir(BinaryTreeNode<T> node, int parentDir) {
		if (node != null) {
			...
			return left != null
					? (right != null ? (left.height > right.height ? -1 : (left.height == right.height ? parentDir : 1))
							: -1)
					: (right != null ? 1 : -1);
		} else
			return -1;
